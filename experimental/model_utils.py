...
def get_exact_2d_pressure(g):
    ...
    dist_top = ((x - 0.5) ** 2 + (y - 0.75) ** 2) ** 0.5
    dist_mid = ((x - 0.5) ** 2) ** 0.5
    dist_bot = ((x - 0.5) ** 2 + (y - 0.25) ** 2) ** 0.5

    p2_top = dist_top ** (1 + alpha)
    p2_mid = dist_mid ** (1 + alpha) + y ** 4 * (1 - y) ** 4 * dist_mid
    p2_bot = dist_bot ** (1 + alpha)

    # Create a list that contains all symbolic expressions
    p2d_sym_list = [p2_bot, p2_mid, p2_top]

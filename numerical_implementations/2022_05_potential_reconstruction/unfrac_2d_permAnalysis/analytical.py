...
class ExactSolution:
    def __init__(self, gb: pp.GridBucket, permeability: float):
        """
        ...
        self.g: pp.Grid = self.gb.grids_of_dimension(2)[0]
        self.d: dict = self.gb.node_props(self.g)

        # Permeability
        self.k: float = permeability

    def __repr__(self) -> str:
        return "Exact solution  object for 2D unfractured problem"
    ...
    def gradp(self, which="cc"):
        ...
        gradp_sym = [sym.diff(self.p("sym"), x), sym.diff(self.p("sym"), y)]

        gradp_fun = [
            sym.lambdify((x, y), gradp_sym[0], "numpy"),
            sym.lambdify((x, y), gradp_sym[1], "numpy"),
        ]

        gradpx_cc = gradp_fun[0](cc[0], cc[1])
        gradpy_cc = gradp_fun[1](cc[0], cc[1])
        gradpz_cc = np.zeros(self.g.num_cells)
    ...
    def u(self, which="cc"):
        ...
        x, y = sym.symbols("x y")
        k = [[self.k, 0], [0, self.k]]  # permeability = identity matrix
        cc = self.g.cell_centers

        u_sym = [
            - k[0][0] * self.gradp("sym")[0] - k[0][1] * self.gradp("sym")[1],
            - k[1][0] * self.gradp("sym")[0] - k[1][1] * self.gradp("sym")[1],
            ]

        u_fun = [
            sym.lambdify((x, y), u_sym[0], "numpy"),
            sym.lambdify((x, y), u_sym[1], "numpy")
        ]

        ux_cc = u_fun[0](cc[0], cc[1])
        uy_cc = u_fun[1](cc[0], cc[1])
        uz_cc = np.zeros(self.g.num_cells)
    ...
    def f(self, which="cc"):
        ...
        f_sym = sym.diff(self.u("sym")[0], x) + sym.diff(self.u("sym")[1], y)

        f_fun = sym.lambdify((x, y), f_sym, "numpy")

        f_cc = f_fun(cc[0], cc[1])

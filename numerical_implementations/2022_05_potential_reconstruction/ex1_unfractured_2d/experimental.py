...
for recon_method in recon_methods:
    estimates = mde.ErrorEstimate(gb, source_list=source_list, p_recon_method=recon_method)
    estimates.estimate_error()
    majorant = estimates.get_majorant()
    te = TrueError(estimates)
    true_error_1 = te.true_error()
    i_eff = majorant / true_error_1
    print(f"Pressure Reconstruction Method: {estimates.p_recon_method}")
    print(f"Majorant: {majorant}")
    print(f"True error: {true_error_1}")
